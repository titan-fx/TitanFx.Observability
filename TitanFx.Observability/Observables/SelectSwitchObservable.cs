...
internal sealed class SelectSwitchObservable<TIn, TOut>(
...
) : IObservable<TOut>
{
    ...
    {
        return new Subscription(source, selector, observer);
    }
    ...
    private sealed class Subscription : IObserver<TIn>, IDisposable
    {
        ...
        {
            _innerSubscription?.Dispose();
            _innerSubscription = _selector(value).Subscribe(_inner);
        }
